  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<WorkflowBloc, WorkflowState>(builder: (context, state) {
      return Customs.WorkflowLayout(
          ...
          onRejected: () {
            _workflowBloc.add(QualityCheckTasksUpdated(
                tasks: _workflowBloc.state.qualityCheckTasks.where((e) => e.isChecked == true).map((e) => e.lpnNbr!).toList(), qcStatus: 'QC Rejected'));
            _workflowBloc.add(SelectAllQualityCheckTasks(isChecked: false));
          },
          child: Expanded(
            child: SfDataGrid(
              ...
              source: WorkflowQualityCheckDataSource(
                data: state.buttonIndex == 0
                    ? state.qualityCheckTasks.where((task) => task.qcStatus == 'QC Pending').toList()
                      ...
                      label: Container(
                          ...
                          child: BlocBuilder<WorkflowBloc, WorkflowState>(
                              ...
                              builder: (context, state) {
                                return Checkbox(
                                    value: state.selectedAllQualityCheckTasks,
                                    onChanged: (value) {
                                      context.read<WorkflowBloc>().add(SelectAllQualityCheckTasks(isChecked: value!));
                                    });
                              }))),
                GridColumn(
                    columnName: 'Facility',
