...
class _InspectionAreaDataSheetState extends State<InspectionAreaDataSheet> {
  ...
  void initState() {
    ...
    if (_warehouseInteractionBloc.state.getInspectionAreaDataState ==
        GetInspectionAreaDataState.initial) {
  ...
  Widget build(BuildContext context) {
    ...
    return Customs.SheetBody(
      ...
      child: Column(
        children: [
          Customs.SheetAppBar(size: size, title: 'Inspection Area',
          ),
          ...
          BlocBuilder<WarehouseInteractionBloc, WarehouseInteractionState>(
            builder: (context, state) {
              ...
              return Skeletonizer(
                ...
                child: SizedBox(height: size.height*0.6, child: ListView.separated(itemBuilder: (context, index) => Customs.MapInfo(size: size, keys: ['ASN', 'PO', 'Item', 'LPN', 'Quantity'], values: [isEnabled ? 'ASN' : state.inspectionArea!.materials![index].asn!,isEnabled ?  'PO'  : state.inspectionArea!.materials![index].po!,isEnabled ?  'Item'  : state.inspectionArea!.materials![index].item!,isEnabled ?  'LPN' : state.inspectionArea!.materials![index].lpn!.toString(),isEnabled ?  'Quantity' : state.receivingArea!.materials![index].quantity!.toString()]), separatorBuilder: (context, index) => Gap(size.height*0.025), itemCount: isEnabled ? 8 : state.inspectionArea!.materials!.length),)
                );
            },
          )
        ],
