...
class _RackDataSheetState extends State<RackDataSheet> {
  ...
  void initState() {
    ...
    if (_warehouseInteractionBloc.state.getRacksDataState ==
        GetRacksDataState.initial) {
  ...
  Widget build(BuildContext context) {
    ...
    return Customs.SheetBody(
      ...
      child: Column(
        children: [
          Customs.SheetAppBar(size: size, title: 'Storage Rack',
          ),
          ...
          BlocConsumer<WarehouseInteractionBloc, WarehouseInteractionState>(
            ...
            builder: (context, state) {
              ...
              return Skeletonizer(
                ...
                child: Column(
                children: [
                  Text(isEnabled ? 'Rack ID' : state.selectedRack!.rackID!, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600),),
                  Gap(size.height*0.02),
                  Customs.MapInfo(size: size, keys: ['Category', 'Number of Bins'], values: [isEnabled ? 'Category' :state.selectedRack!.category!, isEnabled ? '50' :state.selectedRack!.bins!.length.toString()])
                ],
              ));
            },
          )
