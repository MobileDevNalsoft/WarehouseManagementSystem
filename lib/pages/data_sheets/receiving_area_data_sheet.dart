...
class _ReceivingAreaDataSheetState extends State<ReceivingAreaDataSheet> {
  ...
  void initState() {
    ...
    if (_warehouseInteractionBloc.state.getReceivingAreaDataState ==
        GetReceivingAreaDataState.initial) {
  ...
  Widget build(BuildContext context) {
    ...
    return Customs.SheetBody(
      ...
      child: Column(
        children: [
          Customs.SheetAppBar(size: size, title: 'Receiving Area',
          ),
          ...
          BlocBuilder<WarehouseInteractionBloc, WarehouseInteractionState>(
            builder: (context, state) {
              ...
              return Skeletonizer(
                ...
                child: SizedBox(height: size.height*0.6, child: ListView.separated(itemBuilder: (context, index) => Customs.MapInfo(size: size, keys: ['ASN', 'POs', 'LPN', 'Quantity'], values: [isEnabled ? 'ASN' : state.receivingArea!.materials![index].asn!,isEnabled ?  'POs'  : state.receivingArea!.materials![index].pos!.first,isEnabled ?  'LPN' : state.receivingArea!.materials![index].lpns!.toString(),isEnabled ?  'Quantity' : state.receivingArea!.materials![index].quantity!.toString()]), separatorBuilder: (context, index) => Gap(size.height*0.025), itemCount: isEnabled ? 8 : state.receivingArea!.materials!.length),)
                );
            },
          )
        ],
