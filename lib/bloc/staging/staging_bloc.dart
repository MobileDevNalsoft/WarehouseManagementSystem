...
class StagingBloc extends Bloc<StagingEvent, StagingState> {
  StagingBloc({required NetworkCalls customApi})
      : _customApi = customApi,
        super(StagingState.initial()) {
  ...
  void _onGetStagingData(GetStagingData event, Emitter<StagingState> emit) async {
    try {
      ...
      await _customApi
          .get(
        (event.searchText != null && event.searchText != "") ? AppConstants.SEARCH : AppConstants.STAGING_AREA,
        queryParameters: (event.searchText != null && event.searchText != "")
            ? {"search_text": event.searchText, "search_area": "STAGING", "facility_id": '243', "page_num": state.pageNum}
            : {"facility_id": 243},
      )
          .then((value) {
