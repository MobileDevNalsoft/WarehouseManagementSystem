...
class InspectionAreaBloc extends Bloc<InspectionEvent, InspectionAreaState> {
  ...
  Future<void> _onGetInspectionAreaData(GetInspectionAreaData event, Emitter<InspectionAreaState> emit) async {
    try {
      ...
      await _customApi
          .get((event.searchText != null && event.searchText != "") ? AppConstants.SEARCH : AppConstants.INSPECTION_AREA,
          ...
          .then((apiResponse) {
        AreaResponse<ReceivingAreaItem> inspectionAreaResponse =
            AreaResponse.fromJson(jsonDecode(apiResponse.response!.data), (json) => ReceivingAreaItem.fromJson(json));
        if (state.pageNum == 0) {
